...
const CLIENTS = [
  ...
  { name: 'Yasmin Beauty', projectTaskId: '86ahgcemc', services: [{ key: 'crm', label: 'CRM', taskId: 'wdpu2yhedw' }], filePath: 'yasmin-beauty/index.html' },
  ...
  { name: 'Yafa', projectTaskId: 'wdpu2yhhgz', filePath: 'yafa/index.html' },
];
...
async function buildProjectStages(taskId, token, opts) {
  ...
  for (const stageTask of stageTasks) {
    ...
    for (const item of children) {
      ...
      if (isMeetings) {
        // Só due_date — data de início do item não aparece mais nesse nível
        // de propósito (fica só na etapa-mãe e na task-mãe do cliente).
        const meetingDateMs = item.due_date ? Number(item.due_date) : null;
        items.push({
          title: item.name,
          subtitle,
...
function buildClientHtml(clientName, taskId, stages, services, slug, clerkPublishableKey) {
  ...
  .cadastro-card{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:8px 26px;margin-top:22px;box-shadow:0 1px 2px rgba(11,28,51,0.04);}
  ...
  .invoices-card{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:8px 26px;margin-top:22px;box-shadow:0 1px 2px rgba(11,28,51,0.04);}
...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .head{padding-bottom:22px;border-bottom:1px solid rgba(11,28,51,0.14);}
  .logo{height:34px;}
  .head-meta{font-size:12px;color:var(--ink-soft);margin-top:0;}
